// Outline icons, drawn on a 24×24 grid with round caps and joins, coloured with currentColor.
// Most paths are from Lucide (https://lucide.dev, ISC licence). "whatsapp" is our own outline
// drawing in the same style, so it matches the set instead of using the filled brand logo.
const paths = {
  ...
  whatsapp: '<path d="M7.9 20A9 9 0 1 0 4 16.1L2 22Z"/><path d="M9.2 8.4c.3-.3.8-.3 1 .1l.7 1.4c.2.3.1.7-.2 1l-.5.4a5.6 5.6 0 0 0 2.5 2.5l.4-.5c.3-.3.7-.4 1-.2l1.4.7c.4.2.4.7.1 1l-.6.7c-.4.4-1 .5-1.6.3a8 8 0 0 1-4.9-4.9c-.2-.6-.1-1.2.3-1.6z"/>',
  ...
  disc: '<circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="2"/><path d="M12 6a6 6 0 0 0-6 6"/>',
...
// icon("truck") → decorative inline SVG. Pass a label only when the icon is the sole content of a control.
export function icon(name, label, cls = "") {
  const a11y = label ? `role="img" aria-label="${label.replace(/"/g, "&quot;")}"` : 'aria-hidden="true" focusable="false"';
  return `<svg class="icon${cls ? " " + cls : ""}" viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" ${a11y}>${paths[name] || ""}</svg>`;
}
